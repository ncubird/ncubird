			...
			for(var i=0;i<self.raw_data.length;i++){
				...
				switch(self.timeline_search){
					case '#timeline_search_time' :
					    var time = new Date(self.raw_data[i][TIMELINE_UTIL.ROOT_DATA_KEY.START])
						compare_message = (time.getYear()+1900) +'-' + (((time.getMonth()+1) > 9)?(time.getMonth()+1):('0'+ (time.getMonth()+1))) + '-' + (((time.getDate()) > 9)?(time.getDate()):('0'+ (time.getDate()))) + '';
						break;
